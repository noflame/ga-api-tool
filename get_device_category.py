...
# 3. 嘗試獲取令牌並進行 API 調用
def fetch_device_category_data():
        ...
        if response.status_code == 200:
            ...
            if result.get("rows"):
                ...
                for row in result["rows"]:
                    ...
                    print(f"- 裝置類別: {device}, 活躍使用者: {users}")
            return True
        else:
            print("\n請求失敗! 錯誤詳情:")
            try:
...
# ... (run_diagnostics 函數與之前類似，可以複製過來或保持精簡)

# 7. 主函數
if __name__ == "__main__":
    print("===== Google Analytics Data API - 各裝置類別使用者數據測試工具 =====")
    if not os.environ.get('GA4_PROPERTY_ID'):
        print("錯誤：GA4_PROPERTY_ID 環境變數未設定。")
        print('請先設定 GA4_PROPERTY_ID 環境變數再執行此腳本。')
    else:
        success = fetch_device_category_data()
        
        # if not success:
        #     run_diagnostics() # 根據需要決定是否執行診斷
