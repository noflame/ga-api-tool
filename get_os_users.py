        ...
        if response.status_code == 200:
            ...
            if result.get("rows"):
                ...
                for row in result["rows"]:
                    os_name = row.get("dimensionValues", [{}])[0].get("value", "未知作業系統")
                    users = row.get("metricValues", [{}])[0].get("value", "0")
                    print(f"- 作業系統: {os_name}, 活躍使用者: {users}")
            return True
        else:
            print("\n請求失敗! 錯誤詳情:")
            try:
